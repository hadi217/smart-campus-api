...
This API manages campus \*\*Rooms\*\* and the \*\*Sensors\*\* deployed within them, along with a historical log of \*\*SensorReadings\*\*. It is built entirely with JAX-RS (no Spring Boot, no database) using in-memory `ConcurrentHashMap` structures as the data store.
